...
**전체 코드·폴더 검토 후 정리 (죽은 RSS 교체 · 실패 미탐지 구조 수정 · 데드 코드 제거)**

v1.5.17 작업 중 "왜 몇 달간 아무도 실패를 몰랐는가"가 드러나 전수 점검을 수행.

### 1. RSS 소스 7개 중 5개가 죽어 있었음 (심각)

2026-08-19 실측 결과, 폴백 안전망까지 함께 죽은 상태였음.

| 소스 | 상태 |
|------|------|
| Reuters Tech / Business / World | DNS 해석 실패 (Reuters 공개 RSS 종료) |
| 전자신문 `www.etnews.com/rss/rss.xml` | 빈 응답 |
| KBS 폴백 | 빈 응답 — **국내 소스 전멸 시 대비책이 없었음** |
| 연합뉴스 / CNBC Tech | 정상 |

실제 수집원이 2개뿐이었으나 로그에 경고만 남고 방치됨.
국내 4 + 해외 4로 전면 교체, 폴백을 SBS 경제로 변경.
교체 후 8개 전부 정상 (**국내 71건 + 해외 80건** 실측, 기존 31건 대비 4.9배).

- 전자신문은 `rss.etnews.com` 으로 이전됨. **Section903(증권·기업)** 채택 (901/904는 일반·연예 혼재)
- 국내/해외 수집 0건 시 `logger.error` 추가 — 조용히 죽지 않도록

### 2. 실패해도 Actions 가 초록으로 끝나던 구조 (심각)

`main()` 이 어떤 실패에도 종료 코드 0 을 반환.
**v1.5.17 의 Gemini 키 무효화 사고를 사용자가 직접 알아챌 때까지 탐지하지 못한 근본 원인.**

- `_run_morning` / `_run_afternoon` 이 `bool` 반환하도록 변경
- 판정 조건 `success and briefing is not None` — **AI 분석 실패도 실패로 간주**
  (메시지는 시장 데이터만으로 전송되지만 Actions 는 빨간색)
- 실패 시 `sys.exit(1)`
- 검증: 정상 실행 → 0 / Gemini 키 무효 → 1 (양방향 확인 완료)

### 3. FRED US2Y 타임아웃 상시 발생

1976년부터의 전체 시계열(약 13,000행)을 매 실행마다 내려받아 10초 타임아웃 빈발.
`cosd`(관측 시작일) 파라미터로 최근 30일만 요청하도록 변경, timeout 10 → 15초.

### 4. 데드 코드·문서 정리

- `src/config.py` **삭제** — v1.3.0 에서 SECTORS 제거 후 어떤 모듈도 import 하지 않는 주석 3줄짜리 파일
- `requirements.txt` 에서 미사용 `beautifulsoup4` 제거
- 독스트링·프롬프트의 stale 티커 수 "약 75개" → **78개** (미국 76 + 국내 2) 통일
- `.env.example` **신규 생성** — README 가 `cp .env.example .env` 를 안내하는데 파일이 없어 깨진 안내였음
- `README.md` 전면 개편 — 실행 시각(오전 8시 → 06:55), 뉴스 소스(연합뉴스 단독 → 8종), 프로젝트 구조(누락 파일 2종), 실행 모드·수동 트리거·종료 코드 안내 반영

### 검증

- 오전 브리핑 전 파이프라인 실행 + 텔레그램 전송 성공
- 오후 브리핑 경로(수동 트리거 전용, 장기 미검증) 별도 실행 검증 — 정상 (메시지 2,632자)
- 섹터 명칭 정합성 대조: `EXTENDED_TICKERS.sector_kr` ↔ ai_analyzer 프롬프트 목록 **12개 완전 일치**
